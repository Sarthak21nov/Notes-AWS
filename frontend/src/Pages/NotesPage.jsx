...
function NotesPage() {
  ...
  const HandleSubmit = async (e)=>{
    ...
    try{
      const response = await axios.put(`https://notes-aws.duckdns.org/api/v1/notes/${id}`, {
        title, content
      }, {
  ...
  const loadData = async () => {
    ...
    try{
      const response = await axios.get('https://notes-aws.duckdns.org/api/v1/notes/getNotesBasedOnId', {
        withCredentials: true,
        params: {id: id}
      })
      if(response.data.success){
