...
function NotesPage() {
  ...
  const HandleSubmit = async (e)=>{
    ...
    try{
      const response = await axios.put(`ec2-13-203-50-27.ap-south-1.compute.amazonaws.com:5000/api/v1/notes/${id}`, {
        title, content
      }, {
  ...
  const loadData = async () => {
    ...
    try{
      const response = await axios.get('ec2-13-203-50-27.ap-south-1.compute.amazonaws.com:5000/api/v1/notes/getNotesBasedOnId', {
        withCredentials: true,
        params: {id: id}
      })
      if(response.data.success){
